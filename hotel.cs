...
{
    private string PersonPath = "Prefabs/SkelMesh_Bodyguard_01";    // 人物模型地址
    ...
    CreatePerson[] NewPerson = new CreatePerson[420]; // 画面中累计出现420个人

    // Start is called before the first frame update
    void Start()
    {
        // 1.获取数据集
        dt = FilePath.OpenCSV("F:/大四/华科-轨迹预测/data/eth/hotel/pixel_pos.csv");
        // 2. 筛选：取出第0帧数据
        DataTable dtNew = new DataTable();
        dtNew = dt.Copy();
        Count = dt.Columns.Count;
        FrameCount = Convert.ToInt32(dtNew.Rows[0][0].ToString()); // 初始化当前帧数
