...
public class univ_students001 : MonoBehaviour
{
    private string PersonPath = "Prefabs/SkelMesh_Bodyguard_02";    // 人物模型地址
    ...
    private float MoveMax = 0.8f;     // 每秒移动的最大距离
    private bool IsRepeat = false;    // 人物再次出现则该值为 true
    private int stopFrameCount = 20;   // 停止运动的时间，超过该值则删除对应人物
    ...
    CreatePerson[] NewPerson = new CreatePerson[434]; // 画面中累计出现434个人

    // Start is called before the first frame update
    void Start()
    {
        // 1.获取数据集
        dt = FilePath.OpenCSV("F:/大四/华科-轨迹预测/data/ucy/univ/students001/pixel_pos.csv");
        // 2. 筛选：取出第0帧数据
        DataTable dtNew = new DataTable();
        dtNew = dt.Copy();
        Count = dt.Columns.Count;
        FrameCount = Convert.ToInt32(dtNew.Rows[0][0].ToString());
