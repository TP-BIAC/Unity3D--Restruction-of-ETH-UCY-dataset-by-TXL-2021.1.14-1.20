...
public class Track : MonoBehaviour
{
    ...
    private float MoveMax = 0.7f;     // 每次移动的最大距离
    ...
    {
        // 1.获取数据集
        dt = FilePath.OpenCSV("F:/pixel_pos.csv");
        // 2. 筛选：取出第0帧数据
        ...
        for (int i = 0; i < dtNew.Columns.Count; i++)
        {
            ...
            NewPerson[PersonCount].person.transform.position = new Vector3(float.Parse(dtNew.Rows[3][i].ToString()), 0.005f, float.Parse(dtNew.Rows[2][i].ToString()));   // 改变人物位置（x, 0.005, z）
    ...
    void Update()
    {
        ...
        if (FrameCount <= Convert.ToInt32(dt.Rows[0][Count-1].ToString()))
        {
            ...
            if (dtNew.Columns.Count > 0)
            {
                ...
                {
                    // 获取下一个出现的点的位置
                    Vector3 nextPointPosition = new Vector3(float.Parse(dtNew.Rows[3][m].ToString()), 0.005f, float.Parse(dtNew.Rows[2][m].ToString()));
